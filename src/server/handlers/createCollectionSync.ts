...
async function synchroniseRecords<RecordType extends Record>(collection: MXDBSyncedCollection<RecordType>, records: MXDBSyncRequestRecord<RecordType>[], rawLogger: Logger) {
...
export function createCollectionSync<RecordType extends Record>(collection: MXDBSyncedCollection<RecordType>, logger: Logger) {
  return SyncEvents.collection(collection).sync.createSocketHandler(async ({ records }) => {
    ...
    const { updatedRecords, removedRecordIds, newRecordIds } = await synchroniseRecords(collection, records, logger);

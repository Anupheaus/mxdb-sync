...
/** Sync-test policy: harness and settled server audits must not use {@link AuditEntryType.Branched}. */
export function assertNoBranchedEntries(
  ...
  for (let i = 0; i < entries.length; i++) {
    if (entries[i]!.type === AuditEntryType.Branched) {
      violations.push(`${side} record "${recordId}" entry[${i}] is Branched (not allowed in sync-test audits)`);
    }
  }
  return violations;
}
...
function summarizeTruthEntry(e: AuditEntry<SyncTestRecord>): string {
  switch (e.type) {
...
function summarizeServerEntry(e: ServerAuditEntry<SyncTestRecord>): string {
  return summarizeTruthEntry(e as AuditEntry<SyncTestRecord>);
}

function comparePairAtIndex(
  recordId: string,
  index: number,
  t: AuditEntry<SyncTestRecord>,
  s: ServerAuditEntry<SyncTestRecord>,
  ...
  switch (t.type) {
    case AuditEntryType.Created: {
      const tr = (t as { record: SyncTestRecord }).record;
      const sr = (s as { record: SyncTestRecord }).record;
 ...
 * {@link AuditEntryType.Branched} must not appear on either side (sync-test invariant).
 */
export function compareTruthVsServerAudits(
  truth: ReadonlyMap<string, AuditOf<SyncTestRecord>>,
  server: ReadonlyMap<string, ServerAuditOf<SyncTestRecord>>,
